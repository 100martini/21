    ...
    <div className="cl-root">
      ...
      <div className="cl-scroll">
        <Section title="Channels">
          {channels.map(c => {
            const unread = selectedConv?.id === c.id ? 0 : (c.unread_count || 0);
            return (
              <button key={c.id} className={`cl-btn ${selectedConv?.id === c.id ? 'active' : ''} ${unread > 0 ? 'unread' : ''}`} onClick={() => onSelectConv(c)}>
        ...
        <Section title="Direct Messages" action={<button className="cl-add" onClick={onCreateDM}>+</button>}>
          {safeFriends.map(friend => {
            ...
            const unread = (existingConv && selectedConv?.id !== existingConv?.id) ? (existingConv.unread_count || 0) : 0;
            return (
              <button
                key={`f-${friend.id || friend.intraId}`}
                className={`cl-dm ${selectedConv?.id === existingConv?.id ? 'active' : ''} ${unread > 0 ? 'unread' : ''}`}
      ...
      <style>{`
        ...
        .cl-dm-name { font-size: 14px; font-weight: 500; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; color: var(--text-primary); flex: 1; }
        .cl-empty { padding: 12px 10px; font-size: 13px; color: var(--text-muted); }
        .cl-btn.unread .cl-label, .cl-dm.unread .cl-dm-name { font-weight: 700; color: var(--text-primary); }
        .cl-badge {
          flex-shrink: 0;
